...
From the table and the graph (Figure 1 — a standard parabola with the $x$-axis showing $x$ approaching 2 from both sides, and the $y$-axis showing $f(x)$ approaching 4 from below), we see that when $x$ is close to 2 (on either side), $f(x)$ is close to 4. **In fact, we can make $f(x)$ as close to 4 as we like by taking $x$ sufficiently close to 2.**
