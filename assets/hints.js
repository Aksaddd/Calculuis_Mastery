...
window.CalcHints = {
  ...
  "16_section_2.7_rates_exercises": {
    ...
    "7": "For $f(x) = \\sqrt{x}$ at $a = 1$: the difference quotient is $\\dfrac{\\sqrt{1+h} - 1}{h}$. This is $0/0$-indeterminate — rationalize by multiplying top and bottom by $\\sqrt{1+h} + 1$. The numerator becomes $h$, which cancels the $h$ in the denominator. Limit is $\\frac{1}{2}$. Tangent line: $y - 1 = \\frac{1}{2}(x - 1)$.",
    "9": "Use the limit definition with symbolic $a$. $m(a) = \\lim_{h\\to 0} \\dfrac{f(a+h) - f(a)}{h}$ where $f(x) = 3 + 4x^2 - 2x^3$. Expand $(a+h)^2$ and $(a+h)^3$, subtract $f(a)$, divide by $h$, take $h \\to 0$. You'll get $m(a) = 8a - 6a^2$. Then plug in $a = 1$ and $a = 2$ for the two specific tangent lines.",
    "13": "Velocity at $t = 2$ is $y'(2) = \\lim_{h\\to 0} \\dfrac{y(2+h) - y(2)}{h}$ with $y(t) = 40t - 16t^2$. Expand: $y(2+h) = 40(2+h) - 16(2+h)^2 = 80 + 40h - 64 - 64h - 16h^2 = 16 - 24h - 16h^2$. So the difference quotient is $-24 - 16h$, limit $= -24$ ft/s (negative = descending).",
    "17": "The derivative at a point is the *slope* of the tangent line there — steepness of the curve. Look at the graph: where is the curve rising steeply? flat? descending? Order by visual slope — from negative (most downhill) through 0 (horizontal tangent) to positive (uphill). A quick mental sketch comparing slopes is faster than any formal computation.",
    "18": "(a) The tangent line at $x = 5$ has slope $g'(5) = 4$ and passes through $(5, g(5)) = (5, -3)$. Use point-slope: $y - (-3) = 4(x - 5)$. (b) You know two points of the tangent: $(4, 3)$ and $(0, 2)$. Slope is $(3 - 2)/(4 - 0) = 1/4$. So $f'(4) = 1/4$, and $f(4) = 3$ (since the tangent passes through $(4, f(4))$).",
    "19": "Conditions translate to graph features: $f(0) = 0$ ⇒ passes through origin. $f'(0) = 3$ ⇒ ascending steeply through origin. $f'(1) = 0$ ⇒ horizontal tangent at $x = 1$ (local max or min). $f'(2) = -1$ ⇒ descending gently at $x = 2$. Draw a smooth curve that threads these constraints.",
    "23": "Derivative via the limit definition at $x = 2$ for $F(x) = 5x/(1+x^2)$. Compute $F(2+h)$, subtract $F(2) = 2$, form the difference quotient, simplify. After algebra the limit gives $F'(2) = -3/5$ (you should verify by expanding carefully). Tangent line: $y - 2 = -\\frac{3}{5}(x - 2)$.",
    "27": "$f'(t)$ from first principles at a general $t$: $\\dfrac{f(t+h) - f(t)}{h} = \\dfrac{1}{h}\\left[\\dfrac{2(t+h)+1}{(t+h)+3} - \\dfrac{2t+1}{t+3}\\right]$. Get a common denominator, expand the numerator, watch the $h$ factor out. Result: $f'(t) = 5/(t+3)^2$.",
    "35": "Recognize this as a derivative in disguise. The form $\\lim_{h\\to 0} [g(a+h) - g(a)]/h$ is $g'(a)$. Here $g(x) = \\cos x$ and $a = \\pi$: note $\\cos\\pi = -1$, so the numerator is $\\cos(\\pi+h) - (-1) = \\cos(\\pi+h) + 1$. So the limit equals $g'(\\pi) = -\\sin\\pi = 0$.",
    "39": "Initially the soda-refrigerator temperature difference is large, so heat flows fast ⇒ large initial $|dT/dt|$. As the soda cools toward refrigerator temperature, the gap shrinks and the rate slows ⇒ smaller $|dT/dt|$ after an hour. Draw a concave-up decreasing curve approaching the refrigerator temperature asymptotically.",
    "43": "Average rate = $(C(x_2) - C(x_1))/(x_2 - x_1)$. Compute $C$ at each endpoint and subtract. For instantaneous marginal cost at $x = 100$, take the derivative: $C'(x) = 10 + 0.1x$, so $C'(100) = 20$ — meaning producing one additional unit at production-level 100 costs approximately \\$20.",
    "45": "$f'(x)$ is **marginal cost** — the rate of change of total cost with respect to ounces produced. Units: dollars per ounce. $f'(800) = 17$ means at production level 800 oz, producing the next ounce costs about \\$17 more.",
    "47": "$T'(10)$ is the instantaneous rate of change of temperature at $t = 10$ hours past midnight — in other words, how fast temperature is rising or falling at 10 AM, in °F/hr. Estimate using the symmetric difference quotient: $T'(10) \\approx (T(12) - T(8))/(12 - 8)$ using table values.",
    "51": "At $x = 0$: $f'(0) = \\lim_{h\\to 0} \\dfrac{h \\sin(1/h) - 0}{h} = \\lim_{h\\to 0} \\sin(1/h)$. This limit does **not exist** — $\\sin(1/h)$ oscillates wildly between $-1$ and $1$ as $h \\to 0$. So $f$ is continuous at 0 but not differentiable — a clean example of the two concepts being distinct."
  },
